...
Identical root cause to CVE-2026-47890 - a `\r` that breaks SSE field boundaries - but across seven specific call sites spanning three SSE builder classes in the functional MVC and WebFlux APIs. The vulnerability requires attacker-controlled data to reach one of these SSE builder methods (data, comment, id, event, or send). The fix replaces split("\n") with a character-by-character loop that also handles `\r`. The impact is client-side stream corruption only: no server data is leaked, no code is executed. CISA's full-CIA rating has no basis in the code. Spring assigns the same vector and score as CVE-2026-47890, consistent with their similar underlying weakness and impact.
